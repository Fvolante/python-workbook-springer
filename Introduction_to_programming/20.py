...
# user's inputs
pressure = float(input("Inserisci la pressione in Pascals: "))
volume = float(input("Inserisci il volume in litri: "))
temp = float(input("Inserisci la temperatura in gradi Celsius: "))

# convert celsius temp to kelvin
temp = temp + 273.15

# compute substance in moles 
moles = (pressure * volume) / (R * temp)

print("La quantità di gas in moli è %f" %moles)